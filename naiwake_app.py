...
def _is_mae_modoshi(p0_ns: str) -> bool:
    ...
    return any(kw in p0_ns for kw in _MAE_MODOSHI_KWS) or p0_ns.startswith("前期分") or p0_ns == "前期"
...
MODES = {
    ...
    "雑収入（医業外収益）": {
        ...
        "group_map": {                          # 収入先名の強制統合マップ
            "東京リネン": "東京リネンサービス",
            # 半角カナ表記・文字化けパターン（Excel経路 / 変換前）
            "東洋ﾘﾈﾝｻﾌﾟﾗｲ": "東洋リネンサプライ",
            "東洋ﾘﾈﾝｻﾌﾟﾗｲ韓-ｽ料戻り": "東洋リネンサプライ",
            "東洋ﾘﾈﾝｻﾌﾟﾗｲ◆韓-ｽ料戻り": "東洋リネンサプライ",
            "東洋リネンサプライ韓-ｽ料戻り": "東洋リネンサプライ",
            # hankaku_to_zenkaku変換後（CSV経路 / 変換後）
            "東洋リネンサプライ韓-ス料戻り": "東洋リネンサプライ",
            "東洋リネンサプライ◆韓-ス料戻り": "東洋リネンサプライ",
        },
...
def load_jdl_excel(
    ...
    def extract_payee_and_content(desc: str) -> tuple[str, str]:
        ...
        # 「N月分取引内容（支払先）」または「取引内容（支払先）」パターンの処理
        # 全角スペースなし かつ 末尾が全角括弧「（〇〇）」の場合
        if len(parts) == 1:
            m = re.match(r'^(?:\d+月分)?(.*?)（(.+?)）.*', parts[0])
            if m:
                raw_content = _clean_content(normalize_text(m.group(1).strip()))
                raw_payee   = normalize_text(m.group(2).strip())
